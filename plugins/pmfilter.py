...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        btn = InlineKeyboardMarkup(
            [[InlineKeyboardButton("ʀᴇǫᴜᴇsᴛ 📝 ᴀᴅᴍɪɴ", url=OWNER_LNK)]])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
                    InlineKeyboardButton('𖧵 ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ 𖧵', url=f'http://telegram.me/{temp.U_NAME}?startgroup=true')
                ],[
                    InlineKeyboardButton('𖣠‌ ʜᴇʟᴘ 𖣠‌', callback_data='help'),
        ...
        buttons = [[
            InlineKeyboardButton('❃ ɢʀᴏᴜᴘ ❃', url='https://t.me/movies_4uz'),
            InlineKeyboardButton ('⫹⫺ ᴄʜᴀɴɴᴇʟ ⫹⫺', url='https://t.me/movies4u_backup'),
        ],[
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ⇋', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
...
𝗝𝗼𝗶𝗻 𝗼𝘂𝗿 𝗰𝗵𝗮𝗻𝗻𝗲𝗹 𝗯𝗲𝗹𝗼𝘄. 𝗬𝗼𝘂'𝗹𝗹 𝗴𝗲𝘁 𝗻𝗼𝘁𝗶𝗳𝗶𝗲𝗱 𝗮𝘀 𝘀𝗼𝗼𝗻 𝗮𝘀 𝘄𝗲 𝘂𝗽𝗹𝗼𝗮𝗱 𝗶𝘁!""",
    ...
    if not movies:
        ...
        button = [[InlineKeyboardButton(
            "🔰 Request to Admin 🔰", url=f"https://telegram.me/admnsprt_bot")]]
